...
void showMyProject(PNTS MySites) // four points used to define 3 vectors
  ...
  if(scribeText)
    ...
    if(step8) scribeHeader("Step 8: "+titleOfStep[8],line++); //will never be untoggled, thus not included in header
    if(step9) scribeHeader("Step 9: "+titleOfStep[9],line++); //will never be untoggled, thus not included in header
    //scribeHeader("Cubic Method: "+ ((dynamic) ? "Dynamic" : "Static") + " " + cubicStrings[cubicMethod],line++);
    }
...
void doStep3() //
  ...
  M.redistributeInteriorVerticesTowardsNeighbors(0.15);     //*** STUDENT MUST PROVIDE THE CODE FOR THIS METHOD  
  M.redistributeInteriorVerticesTowardsNeighbors(-0.15); 
...
void doStep4() //
  {
  titleOfStep[4] = "Redistribute interior vertices (towards "+ ((showZ) ? "LINK" : "FACES") + "), Press 'Z' to toggle link vs. faces";
  if (showZ)
    {
    M.redistributeInteriorVerticesTowardsLink(0.2);     //*** STUDENT MUST PROVIDE THE CODE FOR THIS METHOD  
    M.redistributeInteriorVerticesTowardsLink(-0.2); // was .6
    }
  else
    {
    M.redistributeInteriorVerticesTowardsFaces(0.2);
    M.redistributeInteriorVerticesTowardsFaces(-0.2);
    }
    guide="My keys: '0'...'9' to activate/deactivate step";
  }
...
void doStep6() //
  ...
  M.smoothenBorderWithTuck(0.1); //*** STUDENT MUST PROVIDE THE CODE FOR THIS METHOD 
  M.smoothenBorderWithTuck(-0.1); 
  guide="My keys: '0'...'9' to activate/deactivate step";
  }

 //====================================================================== PART 7 //*** STUDENT: Phase C (extra credit)
int cubicMethod = 0;
String[] cubicStrings = {"Bezier 3", "Bezier 5", "Neville 3", "Neville 5"};
...
void myKeyPressed()
  ...
  if(key=='Z') showZ ^= true;
  if(key==';') {
    cubicMethod = (cubicMethod < 3) ? cubicMethod + 1 : 0;
  }
  if(key=='\'') dynamic ^= true;
