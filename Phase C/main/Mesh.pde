...
//******************************************************** FOR 3451 PROJECT 4 2020 ********************************************************
  ...
  int leftBeachNeighbor(int cc)
    {
    int commonVertex = V[n(cc)];
    for (int i = 0; i < nc; i++)
      {
      if(o(i) == i && V[p(i)] == commonVertex) return i;
      }
    return 0;
    }
  ...
  int rightBeachNeighbor(int cc)
    {
    int commonVertex = V[p(cc)];
    for (int i = 0; i < nc; i++)
      {
      if(o(i) == i && V[n(i)] == commonVertex) return i;
      }
    return 0;
    }
  ...
  void showBeachNeighbors(float r)
    {
      ...
      {
        cwf(black,1,cyan);
        show(LocationOfCornerDot(leftBeachNeighbor(c)),r);
        cwf(black,1,green);
        show(LocationOfCornerDot(rightBeachNeighbor(c)),r);
      }
    }

  void smoothenBorderWithTuck(float ratio) { // even  border vertex locations
    VCT[] W = new VCT [nv];    // correction vectors
    boolean[] isVisited = new boolean[nv];
    int vertexB, vertexA, vertexC;
    for (int i = 0; i < nv; i++) {
      W[i] = new VCT(0,0);
    }
    for (int corner = 0; corner < nc; corner++) 
      {
        if (isBeachFacing(corner))
          {
            vertexB = V[p(corner)]; // corner
            if (isVisited[vertexB]) { continue; }
            vertexA = V[p(leftBeachNeighbor(corner))];
            vertexC = V[p(rightBeachNeighbor(corner))];
            
            W[vertexB].add(V(G[vertexB], G[vertexA]));
            W[vertexB].add(V(G[vertexB], G[vertexC]));
            W[vertexB].scaleBy(0.5);
            
            isVisited[vertexB] = true;
          }
      }
      for (int i = 0; i < nv; i++)
        {
        //show(G[i], V(ratio, W[i]));
        G[i] = P(G[i], V(ratio, W[i]));
        }
    }

  void smoothenBorderWithCubicPredictor(float ratio) {
    VCT[] W = new VCT [nv];    // correction vectors
    //VCT[] W2 = new VCT[nv];    // testing purposes
    boolean[] isVisited = new boolean[nv];
    int vertexC;
    for (int i = 0; i < nv; i++) {
      W[i] = new VCT(0,0);
    ...
    for (int corner = 0; corner < nc; corner++) 
      {
        if (isBeachFacing(corner))
          {
            vertexC = V[n(corner)]; // vertex being predicted
            if (isVisited[vertexC]) { continue; }
            W[vertexC].add(whichPredictor(cubicMethod, corner));
            isVisited[vertexC] = true;
          }
      }
      for (int i = 0; i < nv; i++)
        {
        //show(G[i], V(ratio, W[i]), blue);
        G[i] = P(G[i], V(ratio, W[i]));
        }
    }
    
    PNT bezier(PNT A, PNT B, PNT D, PNT E, float t) {
      return L(L(L(A,t,B),t,L(B,t,D)), t, L(L(B,t,D),t,L(D,t,E)));
    }
    
    VCT whichPredictor(int cubicMethod, int corner) {
     if(cubicMethod <= 0) {
       return bezier3Cubic(corner);
     } else if (cubicMethod == 1) {
       return bezier5Cubic(corner);
     } else if (cubicMethod == 2) {
       return neville3Cubic(corner);
     } else {
       return neville5Cubic(corner);
     }
    }
    
    VCT bezier3Cubic(int corner) {
      int vertexC = V[n(corner)];
      int leftBorderVert = V[n(leftBeachNeighbor(corner))];
      int rightBorderVert = V[n(rightBeachNeighbor(corner))];
      
      PNT B = P(G[vertexC], V(0.5, V(G[vertexC], G[leftBorderVert])));
      PNT D = P(G[V[p(corner)]], V(0.5, V(G[V[p(corner)]], G[rightBorderVert])));
      
      PNT target;
      if (dynamic) {
        float half, full, cond;
        half = d(G[leftBorderVert],G[vertexC]);
        full = half + d(G[rightBorderVert],G[vertexC]);
        cond = half/full;
        
        target = bezier(G[leftBorderVert], B, D,
          G[rightBorderVert], (cond <= 0.5) ? cond : 1 - cond);
      } else {
        target = bezier(G[leftBorderVert], B, D, G[rightBorderVert], 0.5);
      }
      return V(G[vertexC], target);
    }
    
    VCT bezier5Cubic(int corner) {
      int vertexC = V[n(corner)];
      int leftNeighbor = leftBeachNeighbor(corner);
      int rightNeighbor = rightBeachNeighbor(corner);
      
      PNT A = G[V[n(leftBeachNeighbor(leftNeighbor))]];
      PNT B = G[V[n(leftNeighbor)]];
      PNT D = G[V[n(rightNeighbor)]];
      PNT E = G[V[n(rightBeachNeighbor(rightNeighbor))]];
      ...
      //Knot ratio
        ...
        float half, full, cond;
        half = d(A,B) + d(B,C);
        full = half + d(C,D) + d(D,E);
        cond = half/full;
        
        target = bezier(A, B, D, E,
          (cond <= 0.5) ? cond : 1 - cond);
      } else {
        target = bezier(A, B, D, E, 0.5);
      }
      
      return V(G[vertexC], target);
    }
    
    VCT neville3Cubic(int corner) {
      int vertexC = V[n(corner)];
      int leftBorderVert = V[n(leftBeachNeighbor(corner))];
      int rightBorderVert = V[n(rightBeachNeighbor(corner))];
      ...
      if (dynamic) {
        //Knot ratios
        float half = d(G[leftBorderVert], G[vertexC]);
        float full = half + d(G[rightBorderVert], G[vertexC]);
        float cond = half/full;
        
        target = L(0, G[leftBorderVert], 
          0.5*half/full, B,
          0.5 + (0.5 * half)/full, D,
          1, G[rightBorderVert],
          (cond <= 0.5) ? cond : 1 - cond);
      } else {
        target = L(0, G[leftBorderVert], 0.25, B, 0.75, D, 1, G[rightBorderVert], 0.5);
      }
      
      return V(G[vertexC], target);
    }
    
    VCT neville5Cubic(int corner) {
      int vertexC = V[n(corner)];
      int leftNeighbor = leftBeachNeighbor(corner);
      int rightNeighbor = rightBeachNeighbor(corner);
      ...
      if (dynamic) {
        //Knot ratios
        PNT C = G[vertexC];
        float ab, bc, cd, de, half, full, cond;
        ab = d(A,B);
        bc = d(B,C);
        cd = d(C,D);
        de = d(D,E);
        ...
        target = L(0, A, (ab)/full, B, (half+cd)/full, D, 1, E,
          (cond <= 0.5) ? cond : 1 - cond);
      } else {
        target = L(0, A, 0.25, B, 0.75, D, 1, E, 0.5);
      }
  ...
  void redistributeInteriorVerticesTowardsNeighbors(float ratio)  // even interior vertiex locations
    {    
    VCT[] W = new VCT [nv];    // correction vectors
    int[] count = new int[nv]; // count (for averaging)
    boolean[] isVisited = new boolean[nv];
    int vertexID;
    int currCorner;
    for (int i = 0; i < nv; i++) {
    ...
    for (int corner = 0; corner < nc; corner++) 
      {
        vertexID = V[corner]; // corner
        if (!isInterior[vertexID] || isVisited[vertexID]) {continue;}//continue
        currCorner = corner;
        do
          {
          W[vertexID].add(V(G[vertexID], G[V[n(currCorner)]]));
          count[vertexID]++;
          currCorner = s(currCorner);
          } while (currCorner != corner);
          
        G[vertexID] = P(G[vertexID], V(ratio/count[vertexID], W[vertexID]));
        isVisited[vertexID] = true;
      }
      /*for (int i = 0; i < nv; i++)
        {
        //show(G[i], V(ratio, W[i]));
        //G[i] = P(G[i], V(ratio/count[i], W[i]));
        }*/
    }
    ...
    for (int corner = 0; corner < nc; corner++) 
      {
        ...
        do
          {
          midLink = P(G[V[p(currCorner)]], V(0.5,V(G[V[p(currCorner)]], G[V[n(currCorner)]])));// midpoint of link edge
          W[vertexID].add(V(G[vertexID], midLink));
          count[vertexID]++;
          currCorner = s(currCorner);
          } while (currCorner != corner);
  ...
  void redistributeInteriorVerticesTowardsFaces(float ratio)  // even interior vertiex locations
    {    
    VCT[] W = new VCT [nv];    // correction vectors
    ...
    for (int corner = 0; corner < nc; corner++) 
      {
        ...
        do
          {
          centroid = P(G[vertexID]);
          centroid.add(G[V[p(currCorner)]]); centroid.add(G[V[n(currCorner)]]);
          centroid.mul(1.0/3.0);
          W[vertexID].add(V(G[vertexID], centroid));
          count[vertexID]++;
          currCorner = s(currCorner);
          } while (currCorner != corner);
